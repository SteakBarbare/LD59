...
void main()
{
	...
	//float attenuation = log(max(1.0 - length(v_vPosition.xy - lightInfo.xy) / radius, 0.0) + 1.0) * attenuationIntensity;
	float attenuation =  getAttenuation(v_vPosition - lightInfo.xy, uvs, samplingSize, depth, radius);
	//float attenuation = max(1.0 - length(pos - lightInfo.xy) / radius, 0.0);
	// Normal mapping
	
	//attenuation = floor(attenuation * 2.1) / 2.0;
	vec3 lightDir = normalize(lightInfo.xyz - vec3(v_vPosition.xy, 0)); 
	float d = max(dot(normal, lightDir), 0.0);
	vec3 diffuse = d * lightColor * attenuation * (1.0 - shadow * float(depth <= 0.01));
