if(distance_to_point(objPj.x, y) <= 640){
	var currentTilemap = layer_tilemap_get_id("TileSetAlphaBlood");
	...
	draw_skeleton(sprPjSpine, objPj.currentAnimation, "default", objPj.currentFrame, objPj.x - objBloodReflection.surfaceXview, (objPj.y+yOffset) - objBloodReflection.surfaceYview, objPj.image_xscale, -objPj.image_yscale, 0, -1, 0.6);
	...
	for(var i = 0; i < array_length(reflectingObjects); i++){
		with(reflectingObjects[i]){
			if(object_index != objInvisibleWall){
				...
				if(i == 0)
				{
					yOffset = sprite_height/2 + 2;
				}
				draw_sprite_ext(sprite_index, image_index, x - objBloodReflection.surfaceXview, (y+yOffset) - objBloodReflection.surfaceYview, image_xscale, -image_yscale, 0, -1, 0.6);
